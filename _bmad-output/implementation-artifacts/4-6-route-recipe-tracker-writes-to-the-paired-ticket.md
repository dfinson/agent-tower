...
- [x] Route a completed TaskLink's tracker write through `TrackerWriteService` (AC: #1, #2)
  ...
  - [x] Fires as a tracked fire-and-forget `asyncio.Task` (never awaited inline): `TrackerWriteService.execute` blocks on the operator's approval decision, which must never delay `handle_job_completed`'s own dependent-spawn logic (Story 4.5, AC #1-#3).
...
- [x] Tests (AC: #1, #2, #3)
  - [ ] `backend/tests/unit/test_recipe_service.py`: new `TestHandleJobCompletedTrackerWrite` class —

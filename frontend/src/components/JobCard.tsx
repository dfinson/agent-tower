  ...
  return (
    ...
    >
      <div className="flex justify-between items-start gap-2 mb-1.5">
        {job.title ? (
          <span className="text-sm font-semibold text-primary min-w-0 truncate" title={job.title}>{job.title}</span>
        ) : (
          <span className="text-sm font-semibold text-primary min-w-0 truncate" title={job.id}>{job.id}</span>
        )}
